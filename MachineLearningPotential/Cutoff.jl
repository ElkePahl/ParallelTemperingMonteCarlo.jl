...
function all_angular_measure(a, b, c, r2ab, r2ac, r2bc)
    ...
    thetavec = SVector{3}(
        thetacalc(ab, ac, dis2ab, dis2ac),
        thetacalc(-ab, bc, dis2ab, dis2bc),
        thetacalc(-ac, -bc, dis2ac, dis2bc),
    )
...
function all_angular_measure(a, b, c, r2ab, r2ac, r2bc, thetavec)
    ab, ac, bc = (a - b), (a - c), (b - c)
    dis2ab, dis2ac, dis2bc = sqrt(r2ab), sqrt(r2ac), sqrt(r2bc)
    thetavec[1], thetavec[2], thetavec[3] = thetacalc(ab, ac, dis2ab, dis2ac),
    thetacalc(-ab, bc, dis2ab, dis2bc),
    thetacalc(-ac, -bc, dis2ac, dis2bc)
...
function cutoff_function(r_ij::Float64, r_cut)
    return ifelse(r_ij < r_cut, cutoff_function(r_ij / r_cut), 0.0)
end
