...
function new_angular_symm_vals(
    ...
    θ_new, θ_old = all_angular_measure(
        newposition, position2, position3, rnew_ij, rnew_ik, r2_jk
    ),
    all_angular_measure(position1, position2, position3, r2_ij, r2_ik, r2_jk)

    θ_val_old, θ_val_new = theta_part.(θ_old, Ref(λ), Ref(ζ)),
    theta_part.(θ_new, Ref(λ), Ref(ζ))
...
function calc_swap_symm_val(
    ...
    for index in 1:n1
        if index != atomindex1
            gval1 = calc_one_symm_val(
                dist2_mat[atomindex1, index], f_mat[atomindex1, index], η
            ) #the value of the new Zn atom which will live at index2

            gval2 = calc_one_symm_val(
...
function adjust_radial_symm_val!(
    g_value1, g_value2, rnew_ij, r2_ij, fnew_ij, f2_ij, η, g_norm
)
...
function calc_new_symmetry_value!(
    ...
    g_vector[indexi] = adjust_angular_symm_val!(
        g_vector[indexi], exp_old, exp_new, θ_old[1], θ_new[1], λ, ζ, tpz
    )
    g_vector[indexj] = adjust_angular_symm_val!(
        g_vector[indexj], exp_old, exp_new, θ_old[2], θ_new[2], λ, ζ, tpz
    )
    g_vector[indexk] = adjust_angular_symm_val!(
        g_vector[indexk], exp_old, exp_new, θ_old[3], θ_new[3], λ, ζ, tpz
    )
...
function angular_symmetry_calculation!(
    ...
    if symmetry_function.type_vec == Int(111)
        ...
        symmetry_function.lambda, symmetry_function.zeta,
