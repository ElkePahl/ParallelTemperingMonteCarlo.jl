...
"""
function new_angular_symm_vals(
    newposition,
    position1,
    position2,
    position3,
    ...
    θ_new, θ_old =
        all_angular_measure(newposition, position2, position3, rnew_ij, rnew_ik, r2_jk),
        all_angular_measure(position1, position2, position3, r2_ij, r2_ik, r2_jk)

    θ_val_old, θ_val_new =
        theta_part.(θ_old, Ref(λ), Ref(ζ)), theta_part.(θ_new, Ref(λ), Ref(ζ))

    exp_new, exp_old = exponential_part(η, rnew_ij, rnew_ik, r2_jk, fnew_ij, fnew_ik, f_jk),
    ...
    for index2 = 1:n1
        if index2 != atomindex
            g_new = new_radial_symm_val!(
                new_dist2_vector[index2],
                dist2_mat[atomindex, index2],
                new_f_vector[index2],
                f_matrix[atomindex, index2],
                η,
            )
...
function calc_delta_symm_val!(
    ...
    for j_index = 1:n1
        if j_index != atomindex
            for k_index = (j_index+1):n1
                if k_index != atomindex
                    g_vals = new_angular_symm_vals(
                        newposition,
                        positions[atomindex],
                        positions[j_index],
                        positions[k_index],
                        new_dist2_vector[j_index],
                        new_dist2_vector[k_index],
                        dist2_mat[atomindex, j_index],
                        dist2_mat[atomindex, k_index],
                        dist2_mat[j_index, k_index],
                        new_f_vector[j_index],
                        new_f_vector[k_index],
                        f_matrix[atomindex, j_index],
                        f_matrix[atomindex, k_index],
                        f_matrix[j_index, k_index],
                        η,
                        λ,
                        ζ,
                    )
...
function calc_delta_matrix(
    ...
    for g_index = 1:nrad
        ...
        g_mat[idx:(idx+1), :] = calc_delta_symm_val!(
            ...
            n1,
            n2,
            radsymmfunctions[g_index].eta,
            radsymmfunctions[g_index].G_norm,
        )
    end
    ...
    for g_index = 1:nang
        ...
        g_mat[idx:(idx+2), :] = calc_delta_symm_val!(
            ...
            n1,
            n2,
            angsymmfunctions[g_index].eta,
            angsymmfunctions[g_index].lambda,
            angsymmfunctions[g_index].zeta,
            angsymmfunctions[g_index].tpz,
        )
    end
...
function calc_swap_symm_val(
    ...
    for index = 1:n1
        if index != atomindex1
            gval1 =
                calc_one_symm_val(dist2_mat[atomindex1, index], f_mat[atomindex1, index], η) #the value of the new Zn atom which will live at index2

            gval2 =
                calc_one_symm_val(dist2_mat[atomindex2, index], f_mat[atomindex2, index], η) #the value of the current Zn which will live at index1
    ...
    for index = (n1+1):N
        if index != atomindex2
            gval1 =
                calc_one_symm_val(dist2_mat[atomindex1, index], f_mat[atomindex1, index], η) #the value of the new Zn atom which will live at index2
            gval2 =
                calc_one_symm_val(dist2_mat[atomindex2, index], f_mat[atomindex2, index], η) #the value of the old Zn which will live at index1
...
function calc_swap_symm_val(
        ...
        if j_index != atomindex1
            for k_index = (j_index+1):n1
                if k_index != atomindex1
                    g_vals1 = calc_one_symm_val(
                        positions[atomindex1],
                        positions[j_index],
                        positions[k_index],
                        dist2_mat[atomindex1, j_index],
                        dist2_mat[atomindex1, k_index],
                        dist2_mat[j_index, k_index],
                        f_mat[atomindex1, j_index],
                        f_mat[atomindex1, k_index],
                        f_mat[j_index, k_index],
                        η,
                        λ,
                        ζ,
                    )

                    g_vals2 = calc_one_symm_val(
                        positions[atomindex2],
                        positions[j_index],
                        positions[k_index],
                        dist2_mat[atomindex2, j_index],
                        dist2_mat[atomindex2, k_index],
                        dist2_mat[j_index, k_index],
                        f_mat[atomindex2, j_index],
                        f_mat[atomindex2, k_index],
                        f_mat[j_index, k_index],
                        η,
                        λ,
                        ζ,
    ...
    for k_index = 1:n1
        if k_index != atomindex1
            g_vals = calc_one_symm_val(
                positions[atomindex1],
                positions[atomindex2],
                positions[k_index],
                dist2_mat[atomindex1, atomindex2],
                dist2_mat[atomindex1, k_index],
                dist2_mat[atomindex2, k_index],
                f_mat[atomindex1, atomindex2],
                f_mat[atomindex1, k_index],
                f_mat[atomindex2, k_index],
                η,
                λ,
...
"""
function adjust_radial_symm_val!(
    g_value1,
    g_value2,
    rnew_ij,
    r2_ij,
    ...
    g_value1, g_value2 = adjust_symm_val!(g_value1, rnew_ij, fnew_ij, η, g_norm),
    adjust_symm_val!(g_value2, rnew_ij, fnew_ij, η, g_norm)
    g_value1, g_value2 = adjust_symm_val!(g_value1, r2_ij, -f2_ij, η, g_norm),
    adjust_symm_val!(g_value2, r2_ij, -f2_ij, η, g_norm)
...
function calc_new_symmetry_value!(
    ...
    return g_vector[indexi], g_vector[indexj] = adjust_radial_symm_val!(
        ...
        dist2_mat[indexi, indexj],
        new_f_vector[indexj],
        f_matrix[indexi, indexj],
        η,
        g_norm,
    )
end
...
function calc_new_symmetry_value!(
    ...
    g_vector[indexi] = adjust_angular_symm_val!(
        g_vector[indexi],
        exp_old,
        exp_new,
        θ_old[1],
    ...
    g_vector[indexj] = adjust_angular_symm_val!(
        g_vector[indexj],
        exp_old,
        exp_new,
        θ_old[2],
    ...
    g_vector[indexk] = adjust_angular_symm_val!(
        g_vector[indexk],
        exp_old,
        exp_new,
        θ_old[3],
...
function calc_new_symmetry_value!(
    ...
    return calc_new_symmetry_value!(
        g_vector,
        indexi,
        indexj,
        indexk,
        ...
        position[indexk],
        new_dist2_vector[indexj],
        new_dist2_vector[indexk],
        dist2_mat[indexi, indexj],
        dist2_mat[indexi, indexk],
        dist2_mat[indexj, indexk],
        new_f_vector[indexj],
        new_f_vector[indexk],
        f_matrix[indexi, indexj],
        f_matrix[indexi, indexk],
        f_matrix[indexj, indexk],
        η,
        λ,
        ζ,
        tpz,
...
"""
function radial_symmetry_calculation!(
    g_vector,
    atomindex,
    dist2_mat,
    new_dist2_vector,
...
"""
function angular_symmetry_calculation!(
    g_vector,
    atomindex,
    newposition,
    position,
    ...
    f_matrix,
    new_f_vector,
    symmetry_function::AngularType3,
)
    N = length(g_vector)

    if symmetry_function.type_vec == Int(111)
        η, λ, ζ, tpz = symmetry_function.eta,
        symmetry_function.lambda,
        symmetry_function.zeta,
        symmetry_function.tpz
...
function total_symm!(
    ...
    for g_index = 1:Nrad
        #@views
        g_matrix[g_index, :] = radial_symmetry_calculation!(
            g_matrix[g_index, :],
            atomindex,
            dist2_matrix,
            new_dist_vector,
...
function total_thr_symm!(
    ...
    Threads.@threads for g_index = 1:Nrad
        #@views
        g_matrix[g_index, :] = radial_symmetry_calculation!(
            g_matrix[g_index, :],
            atomindex,
    ...
    Threads.@threads for g_index = 1:Nang
        #@views
        truindex = g_index + Nrad
        g_matrix[truindex, :] = angular_symmetry_calculation!(
            g_matrix[truindex, :],
