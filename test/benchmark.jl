...
begin
    ...
    begin
        global suite["EnergyEvaluation"]["dimer_energy_config"]["ELJEven"] = @benchmarkable dimer_energy_config(d2mat,n_atom,dimer_potential_variables,$(get_eljpot_even())) setup=(config = get_config(get_spherical_bc()); d2mat = get_distance2_mat(config); dimer_potential_variables = get_dimer_vars(config))
        global suite["EnergyEvaluation"]["dimer_energy_config"]["ELJEven/CubicBC"] = @benchmarkable dimer_energy_config(d2mat,n_atom,dimer_potential_variables, r_cut, bc, $(get_eljpot_even())) setup=(bc = get_cubic_bc(); config = get_config(bc); d2mat = get_distance2_mat(config); dimer_potential_variables = get_dimer_vars(config); r_cut = rand()*7)
        global suite["EnergyEvaluation"]["dimer_energy_config"]["ELJEven/RhombicBC"] = @benchmarkable dimer_energy_config(d2mat,n_atom,dimer_potential_variables, r_cut, bc, $(get_eljpot_even())) setup=(bc = get_rhombic_bc(); config = get_config(bc); d2mat = get_distance2_mat(config); dimer_potential_variables = get_dimer_vars(config); r_cut = rand()*7)
        global suite["EnergyEvaluation"]["dimer_energy_config"]["ELJB"] = @benchmarkable dimer_energy_config(d2mat,n_atom,dimer_b_potential_variables,$(get_eljpot_b())) setup=(config = get_config(get_spherical_bc()); d2mat = get_distance2_mat(config); dimer_b_potential_variables = get_eljb_vars(config))
        global suite["EnergyEvaluation"]["dimer_energy_config"]["ELJB/CubicBC"] = @benchmarkable dimer_energy_config(d2mat,n_atom,dimer_b_potential_variables, r_cut, bc, $(get_eljpot_b())) setup=(bc = get_cubic_bc(); config = get_config(bc); d2mat = get_distance2_mat(config); dimer_b_potential_variables = get_eljb_vars(config); r_cut = rand()*7)
    end
    ...
    begin
        ...
        global suite["EnergyEvaluation"]["energy_update!"]["ELJB"] = @benchmarkable energy_update!(trial_pos, index, spherical_config, potvars, d2mat_spherical, new_d2_spherical_vec, 1000., elj_b) setup=(elj_b = get_eljpot_b(); index = get_index(); spherical_config = get_config(get_spherical_bc()); trial_pos = get_trial_pos(spherical_config, index); potvars = get_eljb_vars(spherical_config); d2mat_spherical = get_n_by_n()*7; new_d2_spherical_vec = get_new_d2_spherical_vec(d2mat_spherical, index))
    ...
    begin
        global suite["EnergyEvaluation"]["initialise_energy"]["ELJEven/NVT"] = @benchmarkable initialise_energy(config, d2mat, potvars, ensemble_vars, pot) setup=(config = get_config(get_spherical_bc()); d2mat = get_distance2_mat(config); potvars = get_dimer_vars(config); ensemble_vars = set_ensemble_variables(config, get_nvt()); pot = get_eljpot_even())
        global suite["EnergyEvaluation"]["initialise_energy"]["ELJEven/NPT"] = @benchmarkable initialise_energy(config, d2mat, potvars, ensemble_vars, pot) setup=(config = get_config(get_npt_bc()); d2mat = get_distance2_mat(config); potvars = get_dimer_vars(config); ensemble_vars = set_ensemble_variables(config, get_npt()); pot = get_eljpot_even())
        global suite["EnergyEvaluation"]["initialise_energy"]["ELJB/NVT"] = @benchmarkable initialise_energy(config, d2mat, potvars, ensemble_vars, pot) setup=(config = get_config(get_spherical_bc()); d2mat = get_distance2_mat(config); potvars = get_eljb_vars(config); ensemble_vars = set_ensemble_variables(config, get_nvt()); pot = get_eljpot_b())
        global suite["EnergyEvaluation"]["initialise_energy"]["ELJB/NPT"] = @benchmarkable initialise_energy(config, d2mat, potvars, ensemble_vars, pot) setup=(config = get_config(get_npt_bc()); d2mat = get_distance2_mat(config); potvars = get_eljb_vars(config); ensemble_vars = set_ensemble_variables(config, get_npt()); pot = get_eljpot_b())
        global suite["EnergyEvaluation"]["initialise_energy"]["EmbeddedAtomPotential"] = @benchmarkable initialise_energy(config, d2mat, potvars, ensemble_vars, pot) setup=(config = get_config(get_spherical_bc()); d2mat = get_distance2_mat(config); potvars = get_eam_vars(config); ensemble_vars = set_ensemble_variables(config, get_nvt()); pot = get_eam())
...
begin
    ...
    begin
        global suite["MCRun"]["swap_vars!"]["DimerPotentialVariables"] = @benchmarkable swap_vars!(atom_index, pot_vars) setup=(atom_index = get_index(); config = get_config(get_nvt_bc()); pot_vars = get_dimer_vars(config))
        global suite["MCRun"]["swap_vars!"]["ELJBVariables"] = @benchmarkable swap_vars!(atom_index, pot_vars) setup=(atom_index = get_index(); config = get_config(get_nvt_bc()); pot_vars = get_eljb_vars(config))
        global suite["MCRun"]["swap_vars!"]["EAMVariables"] = @benchmarkable swap_vars!(atom_index, pot_vars) setup=(atom_index = get_index(); config = get_config(get_nvt_bc()); pot_vars = get_eam_vars(config))
        global suite["MCRun"]["swap_vars!"]["RuNNerVariables"] = @benchmarkable swap_vars!(atom_index, pot_vars) setup=(atom_index = get_index(); config = get_config(get_nvt_bc()); pot_vars = get_RuNNer_vars(config))
    end
