...
dir = pwd()
include("$dir/CNA/CNA.jl")
include("$dir/CNA/comparison.jl")
include("$dir/CNA/cluster classification.jl")
...
N_c=10000   #Number of configurations
...
#println()
filedir = "$dir\\CNA"
filedir = "$filedir\\BH_13_0B"
filedir = "$filedir\\BH_13_0B"
savedir = "$filedir\\classify"
for folder in folders
    
    classifyfile = open("$savedir/$folder-classify.txt","w+")
    profilefile = open("$savedir/$folder-profiles_and_energies.txt", "w+")
    open("$filedir/$folder/13_0B_configuration.txt") do f
        for i=1:N_c
            ...
            write(profilefile, "$totalProfiles \n")
            bonds = length(atomicProfiles)
            write(profilefile, "$bonds \n")
            for i in eachindex(atomicProfiles)
                prof = atomicProfiles[i]
