...
export findSymmetries, distance2_rhombic, distance2_cubic, classify_structure, generate_hcp_structure, generate_rhombic_structure, vestaFile, RhombicBC, CubicBC
...
# Function to generate an HCP structure
function generate_hcp_structure(n_atoms::Int)
    r_start = 3.7782  # Desired minimum radius between atoms
    a = r_start * sqrt(2)  # Distance between adjacent atoms in the hexagonal lattice
    c = sqrt(8 / 3) * r_start  # Height of the HCP cell

    Cell_Repeats = round(cbrt(n_atoms / 2))  # HCP unit cell has 2 atoms

    if Cell_Repeats^3 * 2 != n_atoms
        error("Number of atoms not correct for HCP")
    end
    ...
    for i in 0:(Cell_Repeats - 1)
        for j in 0:(Cell_Repeats - 1)
            for k in 0:(Cell_Repeats - 1)
                ...
                y = j * a * sqrt(3) / 2
                z = k * c
                push!(pos, SVector(x, y, z))  # First atom in unit cell
                push!(pos, SVector(x, y + a * sqrt(3) / 3, z + c / 2))  # Second atom in unit cell
            end
        end
    end
    ...
    box_length = Cell_Repeats * a * AtoBohr
    bc = CubicBC(box_length)

    return pos, bc
end

# Function to generate a rhombic (parallelepiped) structure
...
# Function to generate a VESTA file for cubic boundary conditions
...
# Main function to generate HCP or Rhombic structure, classify, and generate VESTA file
function main_structure_vesta(n_atoms::Int, output_dir::String, rCut::Float64, EBL::Float64, structure_type::String)
    if structure_type == "HCP"
        pos, bc = generate_hcp_structure(n_atoms)
        fileName = "hcp_structure"
    elseif structure_type == "Rhombic"
        pos, bc = generate_rhombic_structure(n_atoms)
        fileName = "rhombic_structure"
    else
        error("Invalid structure type. Choose 'HCP' or 'Rhombic'.")
    end

    # Create dummy classifications (all HCP for now)
    classifications = Dict{Vector{Int64}, String}([i] => "HCP" for i in 1:n_atoms)
