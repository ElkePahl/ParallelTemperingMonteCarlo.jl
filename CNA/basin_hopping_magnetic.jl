...
x13_5= [-1.7842827494523337, -2.0947319043914967, -2.0891394653884494, 2.0353068733064275, 0.7701455953565971, 1.002282697443206, -2.3147462493642816, -2.274863151254276, 1.0359575827145693, 2.287298677093446, -1.931209315098458, 0.7454621347775862, -3.3449318469862295, -0.15203921673647372, -0.7047801665600246, -0.4193279972656842, -0.335973810503949, -0.2016673159423398, -2.473105721967891, 0.27665164050296, 2.1975565899459744, -1.4506459429282286, 0.7040786859267868, -2.8022820254699234, -1.8547172751021845, 2.1486226104430517, -0.05403351324142232, 0.05693038875554142, -1.0771366674169873, 2.570040800706996, 0.01280520625635318, -3.261164667997819, -0.08829090104002299, 0.7071316572521139, 2.0390409668565694, -1.3366509071762274, -0.1651747192513829, 1.7999790058936318, 2.422625926469458]
x13_6=[2.389624621872318, -0.8026692767013104, 3.1406665605814705, 0.2804803504855815, 0.6032853963097702, 4.495468321298018, 2.2567378642259697, -1.1054533529191328, 0.004637007614161269, 2.3718153340115067, 1.5194407517673065, 1.3015625536993762, 0.6367199294837299, 0.981326037692487, -1.178260990341802, -0.005771245788403847, 2.747192922667336, 2.279746992308399, -0.4275880097018332, -1.6198026291641037, -0.854156803892127, 0.6596345617600948, -2.7268458409193572, 1.8846718963139155, -1.4197579048864983, -1.5847387971985554, 3.5475449103958536, -2.192043627790516, 1.117057012076164, 3.1122481909626782, -1.8279727985836214, 1.7770985140056161, 0.11673948446347003, -2.6270054451658003, -0.8274782976619545, 0.866870126258019, 0.010397154513783435, -0.007676158985332597, 1.5628210507001936]
x13_666=[2.601918980696449, 1.524534291680147, -2.5145527063097983, 0.7758931998391138, -0.5305383545102557, 1.463920722093082, -0.5722804421738236, -2.919849641538112, -0.4177096796494403, 0.03609942327439854, 0.9721681632979371, -4.136313418154562, -0.8918988117851923, 1.8844291663227866, 0.7706290759060257, 3.451149999954379, -0.05933717648804657, 0.12887341663582846, -1.9479137630591565, -0.6189915792921159, 0.2296148935033968, 0.4485790785699483, 0.17624758110302735, -1.295813612063096, 2.0149870384331097, -2.13756583829189, -1.040140339959919, -1.398910839394151, -1.3449528100832338, -2.8676381140908425, 0.21894148828902574, 2.982683427848003, -1.911831324302732, -2.0619312134930263, 1.3175043821107624, -2.0545609138853838, 1.8361936586043988, 2.144514287041573, 0.46063045862544255]
...
# fitted cut-off of the elj energy in magnetic field
function fitted_2d_peak(theta)
    x=0.15811024*cos(2*theta) -0.12327832*cos(4*theta) +1.58722783
    return x
end

# gradient at the cut-off as a function of theta
function gradient_below_cut_off(theta)
    g=-1.80581036*theta^4+0.31222438*theta^3+2.05421883*theta^2-0.6123577*theta-0.34585987
    return g
end

# dimer energy in magnetic field, elj above cut-off, linear below
function elj2(d,theta)
    t2=cos(2*theta)
    t4=cos(4*theta)
    d_inv=d^(-1)
    d_cut=fitted_2d_peak(theta)
    d_cinv=d_cut^(-1)
    e_cut=d_cut^(-6)*c6*(1+a6*t2+b6*t4)+d_cut^(-8)*(c8*(1+a8*t2+b8*t4)+d_cinv*(c9*(1+a9*t2+b9*t4)+d_cinv*(c10*(1+a10*t2+b10*t4)+d_cinv*(c11*(1+a11*t2+b11*t4)+d_cinv*c12*(1+a12*t2+b12*t4)))))
    if d>=d_cut
        ...
        e=e_cut-gradient_below_cut_off(theta)*(d_cut-d)
    end
...
println("extended lj potential of13 cluster is ",elj(x13_ico))
...
od=OnceDifferentiable(elj,x; autodiff=:forward);
result1 = Optim.minimizer(optimize(od, x, ConjugateGradient(),Optim.Options(g_tol=1e-8)))
println("after minimizatoon ",elj(result1))
for k=1:N
...
for i=1:N
	for j=1:3
		config_1[i,j]=result1[3i-(3-j)]
	end
end
println("initial total CNA profile of config 1 is")
println(CNA(config_1, N, dcut, true, 1)[1])
println(CNA(config_1, N, dcut, true, 1)[2])

config_2=Array{Float64}(undef,N,3)
for i=1:N
	for j=1:3
		config_2[i,j]=x13_hcp[3i-(3-j)]*3.0
	end
end
println("initial total CNA profile of config 2 is")
println(CNA(config_2, N, dcut, true, 3.06)[1])
println(CNA(config_2, N, dcut, true, 3.06)[2])

totalProfile_1 = Dict{String,Int}() 
totalProfile_1 = CNA(config_1, N, dcut, true, 3.06)[1]
atomicProfile_1 = [Dict{String,Int}() for i in 1:N]
atomicProfile_1 = CNA(config_1, N, dcut, true, 3.06)[2]

println(similarityScore_one(1, 2, [totalProfile_1,CNA(config_2, N, dcut, true, 3.06)[1]],[atomicProfile_1,CNA(config_2, N, dcut, true, 3.06)[2]],"total",N,))
...
for i=1:BHstep
    ...
    local od
    od=OnceDifferentiable(elj,x; autodiff=:forward);
    println("enenrgy before minimization is ", elj(x))
    result2 = Optim.minimizer(optimize(od, x, ConjugateGradient(),Optim.Options(g_tol=1e-6)))
    e_end=elj(result2)
    ...
    if alert(result2, radius+1)>=1
        ...
        #for k=1:N
            #println(10," ",x[3k-2]," ",x[3k-1]," ",x[3k])
        #end
        energy=e_end=final_e_end=elj(result1)
    else
        ...
        similarities[count_cluster]=similarityScore_one(1, 2, [totalProfile_1,CNA(config, N, dcut, true, 3)[1]],[atomicProfile_1,CNA(config, N, dcut, true, 3)[2]],"total",N,)
        #println(CNA(config, N, 4.146, true, 3)[1])
        #println(CNA(config, N, 1.3549, false, 3)[2])
        similarity=similarityScore_one(1, 2, [totalProfile_1,CNA(config, N, dcut, true, 3.06)[1]],[atomicProfile_1,CNA(config, N, dcut, true, 3.06)[2]],"total",N,)
        println(similarity)
        println(CNA(config, N, dcut, true, 3)[1])
        if similarity>0.499 && similarity<1.0
            open("/Users/tiantianyu/Downloads/13_similarity_0.5.txt","a") do io
                println(io,final_e_end)
                println(io)
                println(io,similarity)
                println(io)
                println(io,CNA(config, N, dcut, true, 3)[1])
                println(io)
                #for k=1:N
                    #println(io, 10," ",x[3k-2]," ",x[3k-1]," ",x[3k])
                #end
                #println(io)
                #for k=1:N
                    #println(io, 10," ",result2[3k-2]," ",result2[3k-1]," ",result2[3k])
                #end
                #println(io)
            end
        elseif similarity>0.4 && similarty<1.0
            open("/Users/tiantianyu/Downloads/13_similarity_0.4.txt","a") do io
                println(io,final_e_end)
                println(io)
                println(io,similarity)
                println(io)
        ...
        elseif similarity>0.35 && similarity<1.0
            open("/Users/tiantianyu/Downloads/13_similarity_0.35.txt","a") do io
                println(io,final_e_end)
                println(io)
                println(io,similarity)
                println(io)
            ...
            #open("/Users/tiantianyu/Downloads/13_similarity_0.3.txt","a") do io
                #println(io,final_e_end)
                #println(io)
                #println(io,similarity)
                #println(io)
