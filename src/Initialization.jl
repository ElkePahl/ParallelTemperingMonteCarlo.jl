...
"""
function initialisation(mc_params::MCParams,temp::TempGrid,start_config1::Config,start_config2::Config,potential::Ptype,ensemble::Etype) where Ptype <: AbstractPotential where Etype <:AbstractEnsemble
    ...
    mc_states = [MCState(temp.t_grid[i], temp.beta_grid[i],start_config1,ensemble,potential) for i in 1:mc_params.n_traj]
    # shared_config1 = start_config[1]
    # shared_config2 = start_config[2]
    #mc_states = Array{MCState}(undef, mc_params.n_traj)
    for i in 17:32 
        mc_states[i] = MCState(temp.t_grid[i], temp.beta_grid[i],start_config2,ensemble,potential)
    end
    # for i in 17:32
