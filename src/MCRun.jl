...
"""
function initialise_histograms!(mc_params,mc_states,results,r_max; full_ham = true,e_bounds = [0,0])    
    ...
    #elseif typeof(mc_states[1].config.bc)<=PeriodicBC
        #r_max = mc_states[1].config.bc.box_length*0.5*3^0.5  #diagonal of the box
        #delta_r = r_max/results.n_bin/5 #we want more bins for the RDFs
    #end
...
"""
function ptmc_cycle!(mc_states,results,r_max,move_strat, mc_params, pot, ensemble ,n_steps ,a ,v ,r, save_ham, save, i,save_dir ;delta_en_hist=0.)
    ...
    if save_ham == false
        ...
        updaterdf!(mc_states,results,(r_max/(results.n_bin*5)))
...
function ptmc_run!(mc_states, move_strat, mc_params, pot, ensemble, results,r_max; save_ham::Bool = true, save::Bool=true, restart::Bool=false,restartindex::Int64=0,save_dir = pwd())
    ...
    #re-set counter variables to zero
        ...
        if save_ham == false
            ...
            delta_en_hist = initialise_histograms!(mc_params,mc_states,results, r_max,full_ham=false,e_bounds=ebounds)
    ...
    if restart == false
        ...
        for i = 1:mc_params.mc_cycles
            if save_ham == false
                ...
                @inbounds ptmc_cycle!(mc_states,results,r_max,move_strat, mc_params, pot, ensemble ,n_steps ,a ,v ,r, save_ham, save, i,save_dir;delta_en_hist=delta_en_hist)
            else
                @inbounds ptmc_cycle!(mc_states,results,r_max,move_strat, mc_params, pot, ensemble ,n_steps ,a ,v ,r, save_ham, save, i,save_dir)
            end
    ...
    if save_ham == true
        ...
        delta_en_hist= initialise_histograms!(mc_params,mc_states,results,r_max)
