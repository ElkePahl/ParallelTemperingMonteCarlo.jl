...
max_vchange = 0.01    # For periodic boundary only
...
count_acc = zeros(n_traj)        #total count of acceptance of displacement
...
count_v_acc = zeros(n_traj)        #total count of acceptance of volume change, for periodic boundary only
count_v_acc_adj = zeros(n_traj)
...
function mc_step_atom_cbc!(config, beta, dist2_mat, en_atom_mat, en_tot, NAtoms, i_atom, max_displacement, max_vchange, count_d_acc, count_d_acc_adj, count_v_acc, count_v_acc_adj, pot1)
    ...
    if select_move <= NAtoms
        ...
        en_changed = dimer_energy_config(dist2_new_config, NAtoms, elj_ne)
        if metropolis_condition(en_tot, en_changed, beta) >= rand()
            config=trial_config
            en_tot=en_changed
...
function mc_step_atom_eq_cbc!(config, beta, dist2_mat, en_atom_mat, en_tot, NAtoms, i_atom, max_displacement, max_vchange, count_d_acc_adj, count_v_acc_adj, pot1)
    #displace atom, until it fulfills boundary conditions
    #select_move=rand(1:NAtoms+1)
    select_move=33
    if select_move <= NAtoms
...
dist2_mat_0=get_distance2_mat(conf_ne32_pbc)
...
for i=1:n_traj
    config[i]=Config(copy(conf_ne32_pbc.pos),bc_ne32_pbc)
...
for i=1:equilibrium
    for j=1:n_traj                   #number of atoms
        for k=1:NAtoms            #trajetcories
            ...
            config[j], dist2_mat[j], en_tot[j], count_acc_adj[j], count_v_acc_adj[j] = mc_step_atom_eq_cbc!(config[j], temp.beta_grid[j], dist2_mat[j], en_atom_mat[j], en_tot[j], NAtoms, i_atom, displ_param.max_displacement[j], max_vchange[j], count_acc_adj[j], count_v_acc_adj[j], elj_ne)
        end
    end
...
for i=1:mc_cycles
    for j=1:n_traj                    #number of atoms
        for k=1:NAtoms            #trajetcories
            ...
            config[j], dist2_mat[j], en_tot[j], count_acc[j], count_d_acc_adj[j], count_v_acc[j], count_v_acc_adj[j] = mc_step_atom_cbc!(config[j], temp.beta_grid[j], dist2_mat[j], en_atom_mat[j], en_tot[j], NAtoms, i_atom, displ_param.max_displacement[j], max_vchange[j], count_acc[j], count_acc_adj[j], count_v_acc[j], count_v_acc_adj[j], elj_ne)
        end
