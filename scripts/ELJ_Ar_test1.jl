...
# Perform optimization using the ELJ potential
function optimize_structure(atoms, coeff, box_length)
    ...
    result = Optim.minimizer(optimize(od, x, ConjugateGradient(), Optim.Options(g_tol=1e-9)))
    minimized_atoms = rebuild_atoms(result, atoms, bc)
...
for file_path in glob("*.dat", input_dir)
    ...
    # Modify the filename to remove the ".dat" extension and write only ".xyz"
    minimized_xyz_path = joinpath(output_dir_minimized, "minimized_" * replace(basename(file_path), ".dat" => "") * ".xyz")
