...
input_dir = "/Users/samuelcase/Dropbox/PTMC_Lit&Coding/Sam_Results/Data"
...
for file_path in glob("*.dat", input_dir)
    ...
    for (cycle_index, atoms, bc) in structures
        ...
        # Generate filenames using the cycle index
        ...
        if bc isa EMCubicBC
            println("Processed cycle: $cycle_index with Box Length: ", bc.box_length)
        elseif bc isa EMRhombicBC
            println("Processed cycle: $cycle_index with Box Width: ", bc.box_width, ", Box Height: ", bc.box_height)
        end
    end
end
